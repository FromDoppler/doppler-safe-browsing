properties { 
  ...
  $majorVersion = "0.1"
  $majorWithReleaseVersion = "0.1.0-alpha1"
...
task Build -depends Clean {
  ...
  Copy-Item -Path $baseDir\LICENSE -Destination $workingDir\
  mkdir "$workingDir\Build" -Force
  Copy-Item -Path $buildDir\install.ps1 -Destination $workingDir\Build\

  $xml = [xml](Get-Content "$workingSourceDir\MakingSense.SafeBrowsing\MakingSense.SafeBrowsing.csproj")
  ...
  $xml.save("$workingSourceDir\MakingSense.SafeBrowsing\MakingSense.SafeBrowsing.csproj")

  $projectPath = "$workingSourceDir\MakingSense.SafeBrowsing\MakingSense.SafeBrowsing.csproj"
...
task Package -depends Build {
  foreach ($build in $script:enabledBuilds)
  {
    ...
    $sourcePath = "$workingSourceDir\MakingSense.SafeBrowsing\bin\Release\$finalDir"
  ...
  if ($buildNuGet)
  {
    ...
    exec { & $script:msBuildPath "/t:pack" "/p:IncludeSource=true" "/p:Configuration=Release" "/p:TargetFrameworks=`"$targetFrameworks`"" "$workingSourceDir\MakingSense.SafeBrowsing\MakingSense.SafeBrowsing.csproj" }

    mkdir $workingDir\NuGet
    move -Path $workingSourceDir\MakingSense.SafeBrowsing\bin\Release\*.nupkg -Destination $workingDir\NuGet
  }
...
{
  $projectPath = "$workingSourceDir\MakingSense.SafeBrowsing.sln"
...
{
  $projectPath = "$workingSourceDir\MakingSense.SafeBrowsing.Tests\MakingSense.SafeBrowsing.Tests.csproj"
  $location = "$workingSourceDir\MakingSense.SafeBrowsing.Tests"
...
function NUnitTests($build)
{
  ...
  Write-Host
  robocopy "$workingSourceDir\MakingSense.SafeBrowsing.Tests\bin\Release\$testDir" $testRunDir /MIR /NFL /NDL /NJS /NC /NS /NP /XO | Out-Default
  ...
  try
  {
    ...
    exec { & $nunitConsolePath\tools\nunit3-console.exe "$testRunDir\MakingSense.SafeBrowsing.Tests.dll" --framework=$framework --result=$workingDir\$testDir.xml | Out-Default } "Error running $testDir tests"
  }
  finally
  {
    Set-Location $baseDir
